...
char tstr[14];
char tzxbuf[10]={ 'Z','X','T','a','p','e','!', 0x1A, 1, 10 };
char cpcbuf[5]={ 0x33, 1, 0, 0x15, 1 };
...
char buf2[255];				// Buffer for file writes
...
if (c==temp[len-1]) return("û "); else return("ù ");
}
...
void SavePureData(char *data, int len, int sync)
	{
	...
	buf2[5]=(char) posbit;
	buf2[6]=(char) (((int) ((pausev+0.0005)*1000.0))&0xff);
	buf2[7]=(char) (((int) ((pausev+0.0005)*1000.0))>>8);
	buf2[8]=(char) (len&0xff); buf2[9]=(char) ((len>>8)&0xff);
	buf2[10]=(char) ((len>>8)>>8);
	write(ofh,buf2,11); write(ofh,data,len);
	if (sync)	// Write the Mid-Block SYNC ?
		...
		write(ofh,buf2,2);	// Write the mid-block SYNC , ENDIAN depandable !!!
		write(ofh,slockmidsync,21*2*2);  // ENDIAN depandable !!!
		}
	}
...
if (diload && (block==3 || block==4))
	...
	buf2[0]=0x12;
	buf2[1]=(char) (adipilot&0xff); buf2[2]=(char) (adipilot>>8);
	buf2[3]=(char) (dipilot&0xff); buf2[4]=(char) (dipilot>>8);
	write(ofh,buf2,5);
	// Save the PILOT
	buf2[0]=0x12;
	buf2[1]=(char) (apilot&0xff); buf2[2]=(char) (apilot>>8);
	buf2[3]=(char) (lst_pilot&0xff); buf2[4]=(char) (lst_pilot>>8);
	write(ofh,buf2,5);
	// Save the SYNC
	buf2[0]=0x13;
	buf2[1]=1;
	buf2[2]=(char) (sync1&0xff); buf2[3]=(char) (sync1>>8);
    write(ofh,buf2,4);
	// Save the DATA
	SavePureData(temp,posbyte,0);
	// And the group end
...
if ((Diff(ahp0,HP_BIT0)<=STD_TRES) && (Diff(ahp1,HP_BIT1)<=STD_TRES) &&
	...
	buf2[0]=0x10;
	buf2[1]=(char) (((int) ((pausev+0.0005)*1000.0))&0xff);
	buf2[2]=(char) (((int) ((pausev+0.0005)*1000.0))>>8);
	buf2[3]=(char) (posbyte&0xff); buf2[4]=(char) (posbyte>>8);
	write(ofh,buf2,5); write(ofh,temp,posbyte);
	}
else
	{
	...
	buf2[13]=(char) posbit;
	buf2[14]=(char) (((int) ((pausev+0.0005)*1000.0))&0xff);
	buf2[15]=(char) (((int) ((pausev+0.0005)*1000.0))>>8);
	buf2[16]=(char) (posbyte&0xff); buf2[17]=(char) ((posbyte>>8)&0xff);
	buf2[18]=(char) ((posbyte>>8)>>8);
	write(ofh,buf2,19); write(ofh,temp,posbyte);
	}
}
...
buf2[0]=0x13;
buf2[1]=14;
write(ofh,buf2,2); write(ofh,slock12sync,14<<1);   // ENDIAN depandable !!!
//
GetActualStandardData();
...
if (slock==3 && slockvar==2 && numblock==1)
	...
	buf2[0]=0x12;
	buf2[1]=(char) (noisepulse&0xff); buf2[2]=(char) (noisepulse>>8);
	buf2[3]=(char) (noiselen&0xff); buf2[4]=(char) (noiselen>>8);
	write(ofh,buf2,5);
	buf2[0]=0x20;
	buf2[1]=(char) (300&0xff); buf2[2]=(char) (300>>8);
	write(ofh,buf2,3);
	}
if (slock!=2)
	{
	...
	buf2[0]=0x12;
	buf2[1]=(char) (apilot&0xff); buf2[2]=(char) (apilot>>8);
	buf2[3]=(char) (pilot&0xff); buf2[4]=(char) (pilot>>8);
	write(ofh,buf2,5);
	// Save the big SYNC (plus 6 bits of first byte) as Sequence of Pulses
	buf2[0]=0x13;
	buf2[1]=28;
	write(ofh,buf2,2); write(ofh,slock34sync,28<<1);  // ENDIAN depandable !!!
	}
else
	{
...
printf(
"%c%s P-%4d,%4d S-%4d/%4d 0-%4d,1-%4d F-%2X B%d L-%5d C%s",
...
//			printf("Start: %d   End: %d\n",start,endb);
		...
		if (!NoData)			// Got some Actual Data from conversion
			...
			if (tape)		// Write the .TAP block
				...
				write(ofh,buf2,2); write(ofh,temp,posbyte);
				}
...
{
printf("\nZXTape Utilities - VOC to TZX Converter v0.54b\n");
if (argc<2)
	{
	printf("\nUsage: VOC2TZX [switches] INPUT.VOC [OUTPUT.TZX | OUTPUT.TAP]\n");
	...
	if (argv[n][0]=='/')
		...
		switch (argv[n][1])
			...
			case 's':	switch (argv[n][2])
						...
						break;
			case 'f':	if (strcmp(argv[n],"/force")) invalidoption(argv[n]);
					 	forcep=getnumber(argv[n+1]); force=1; n++; break;
			default :	invalidoption(argv[n]);
			}
		}
	else

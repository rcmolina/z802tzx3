...
/******************************************************
        ...
        ZX Spectrum SNAPSHOT to TZX Tape Converter

                         v 1.2

             by Tomaz Kac  (tomcat@sgn.net)

  .SNA Support by Simon Owen (simon.owen@simcoupe.org)
...
//	  	  sprintf(out_filename+last,"_%db.tzx",speed_value);	
		...
		{
		case 0:		//1364
			//out_filename[last+1]='1';out_filename[last+2]='3';out_filename[last+3]='6';out_filename[last+4]='4';
			sprintf(out_filename+last+1,"%d",1364);
			break;
		
		case 1:		//2250
			//out_filename[last+1]='2';out_filename[last+2]='2';out_filename[last+3]='5';out_filename[last+4]='0';	  
			sprintf(out_filename+last+1,"%d",2250); 	  	  
			break;
		
		case 2:		//3000
			//out_filename[last+1]='3';out_filename[last+2]='0';out_filename[last+3]='0';out_filename[last+4]='0';	 
			sprintf(out_filename+last+1,"%d",3000);  	   
			break;
		
		case 3:		//3500
			//out_filename[last+1]='3';out_filename[last+2]='5';out_filename[last+3]='0';out_filename[last+4]='0';
			sprintf(out_filename+last+1,"%d",3500);	 
			break;
		
		case 4:		//3600
			//out_filename[last+1]='3';out_filename[last+2]='6';out_filename[last+3]='0';out_filename[last+4]='0';
			sprintf(out_filename+last+1,"%d",3600);	 
			break;
		
		case 5:		//3700
			//out_filename[last+1]='3';out_filename[last+2]='7';out_filename[last+3]='0';out_filename[last+4]='0';
			sprintf(out_filename+last+1,"%d",3700);	 
			break;
		
		case 6:		//3800
			//out_filename[last+1]='3';out_filename[last+2]='8';out_filename[last+3]='0';out_filename[last+4]='0';
			sprintf(out_filename+last+1,"%d",3800);	 
			break;
		
		case 7:		//3900
			//out_filename[last+1]='3';out_filename[last+2]='9';out_filename[last+3]='0';out_filename[last+4]='0';
			sprintf(out_filename+last+1,"%d",3900);	 
			break;
		
		case 8:		//4000
			//out_filename[last+1]='4';out_filename[last+2]='0';out_filename[last+3]='0';out_filename[last+4]='0';
			sprintf(out_filename+last+1,"%d",4000);	 
			break;
		
		case 9:		//4100
			//out_filename[last+1]='4';out_filename[last+2]='1';out_filename[last+3]='0';out_filename[last+4]='0';
			sprintf(out_filename+last+1,"%d",4100);	 
			break;
		
		case 10:	//4500
			//out_filename[last+1]='4';out_filename[last+2]='5';out_filename[last+3]='0';out_filename[last+4]='0';
			sprintf(out_filename+last+1,"%d",4500);	 
			break;
		}
		...
		sprintf(out_filename+last+5,"b.tzx");
	}
}
...
*/	  	  	  	  	  
				...
				case 'l':
					i++;
					sscanf(argv[i], "%[^\0]\0", loader_name);
					loader_name[8] = 0;	// Can't be bigger than 8 chars !
					break;
					// Output Filename
				case 'o':
					i++;
					sscanf(argv[i], "%[^\0]\0", out_filename);
					break;
					// External Loading Screen
				case '$':
					i++;
					external = true;
					sscanf(argv[i], "%[^\0]\0", external_filename);
					break;
...
/* Max speed: a = 1, b = 0 (bd = 2) => 279 + 32 = 311 (hb0 = 156 T) => 156 T x 3 = 468 average bit length => 3.500.000/468 = 7479 bps    */
...
void create_main_header()
{
	...
	{
		tzx_header[14+9+i] = loader_name[i];
	}

	// Fill in the length of data
	tzx_header[14+17] = len&255;
	tzx_header[14+18] = len>>8;

	int var = (LOADERPREPIECE-1)-21;	// Varables start here
	// Fill in proper variable area
	tzx_header[14+21] = var&255;
	tzx_header[14+22] = var>>8;

	// Calc checksum
	tzx_header[14+23] = calc_checksum(tzx_header+14+5, 18);
}
...
void create_main_data()
{
	...
	tzx_header_data[14+2] = len&255;
	tzx_header_data[14+3] = len>>8;

	tzx_header_data[14+4] = 0;
...
//	snap_bin[data_pos]= calc_checksum(snap_bin+14+5, data_pos-14-5);  // need to recalc this in the end !!!
	...
	tzx_turbo_head[11]      = (word)((dword)3500000 * 0.5 / turbo_vars[speed_value]._LenPilot) |1 &255;
	...
	for (int i=0; i < 8; i++)
	{
		...
		else
		{
			...
			for (int j=0; j < 16384-768 && !load; j++)
			{
				if (snap.page[i][j] != 0)
				{
					load = true;
			...
			for (int j=16384-768; j < 16384 && !load_768; j++)
			{
				if (snap.page[i][j] != 0)
				{
					load_768 = true;
...
*/				
	...
	if (load_768)
	{
		...
		tzx_header_data[14 + 2] = (768+2)&255;	 // Length of extra block
		tzx_header_data[14 + 3] = (768+2)>>8;
		tzx_header_data[14 + 4] = 0;
		tzx_header_data[14 + 5] = 0x55;	 	 // Flag is 0x55
	...
	snap_bin[main_checksum]= calc_checksum(snap_bin+14+5, main_checksum-14-5);  // need to recalc this in the end!!!
}

...
dir=(char *) malloc(flen>>1);
if (raw==NULL || dir==NULL) { close(fh); return(2); }
if (!read(fh,buf,4)) { close(fh); return(3); } buf[4]=0;
if (strcmp(buf,"RIFF")) { close(fh); return(3); }

lseek(fh, 0, SEEK_SET);
if (!read (fh,buf,44)) return(1);	 //skip wav file header (44bytes)

//rawlen= flen -44;
rawlen= (buf[40]&0xFF) + 256*(buf[41]&0xFF) +  256*256*(buf[42]&0xFF) + 256*256*256*(buf[43]&0xFF);

//printf("%d %x %x %x %x\n", rawlen, buf[40], buf[41], buf[42], buf[43]);
rate = (buf[24]&0xFF) + 256*(buf[25]&0xFF);
printf("\nImplicit Sampling Rate: %5i Hz\n",rate);
printf("Loading File (%d bytes) ...\n",flen);
//printf("len %d\n", len);
...
while (p < flen)
{
	plen = read(fh, raw+p-44, 256);
	p= p + plen;  	   
}

//printf("raw %x %x %x %x %x\n", *(raw+0), *(raw+1), *(raw+6), *(raw+7), *(raw+rawlen-1) );
if (silence) printf("-- Warning: Silence blocks in file (Pauses will be wrong) !\n");
printf("Actual RAW size of file in memory: %d bytes\n",rawlen);
if (!endb) endb=p-44-1;
close(fh);
...
if (argc<2)
	{
	printf("\nUsage: DIRECT [switches] INPUT.WAV [OUTPUT.TZX]\n");
